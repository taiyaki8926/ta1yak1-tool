...
_sage_const_2 = Integer(2); _sage_const_0p3 = RealNumber('0.3'); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_0p29 = RealNumber('0.29'); _sage_const_7 = Integer(7); _sage_const_60 = Integer(60); _sage_const_4 = Integer(4); _sage_const_0p292 = RealNumber('0.292')
import sys
...
def wiener(c, e, N):
    def recover(e, N):
        ...
        cf = f.convergents()
        G = ZZ['x']; (x,) = G._first_ngens(1)
        for index, k in enumerate(cf[_sage_const_1 :]):
            d0 = k.denominator()
            k = k.numerator()
            if k != _sage_const_0  and (e * d0 - _sage_const_1 ) % k == _sage_const_0 :
                
                phi = (e*d0 - _sage_const_1 ) //k
                s = (N-phi+_sage_const_1 )
                f = x**_sage_const_2  - s*x + N
                b = f.discriminant()
                if b > _sage_const_0  and b.is_square():
                    d = d0
                    
                    roots = list(zip(*f.roots()))[_sage_const_0 ]
                    if len(roots) == _sage_const_2  and prod(roots) == N:
                        return d
                else:
                    continue
...
"""
def boneh_durfee(c, e, N, delta=_sage_const_0p29 , debug=False):
    ############################################
    # Config
    ##########################################
